...
# ═══════════════════════════════════════════════════════════════════
# Multi-seed: 3 independent Phase1+Phase2+Phase3 runs per horizon.
# Purpose (per user request): (1) quantify the seed-to-seed variance the
# paper's own Sec 8.3 already flags (+/-3pp at H=21d) instead of asserting
# it, and (2) report an ensemble (averaged-prediction) result -- a
# principled way to get a more robust single number that is NOT
# hyperparameter search against the test set (no test-set-driven choices
# are made here).
N_SEEDS = 3

PREDS = {}          # H -> ensemble-based dict (same shape as before, for figures/CSV)
SEED_STATS = {}      # H -> per-seed metric lists + mean/std, for transparency
...
for H in HORIZONS:
    ...
    seed_preds = []   # per-seed dict: pred_raw, anil_pred_raw, pred_tta, pred_tta_bias
    ...
    for s_idx in range(N_SEEDS):
        ...
        print(f"  --- seed {s_idx+1}/{N_SEEDS} ({_seed}) ---")
        ...
        bar = tqdm(range(1, EPOCHS+1), desc=f"H={H}d s{s_idx}")
        for ep in bar:
            model.train()
            for Xb, yb in dl:
        ...
        if s_idx == 0:
            torch.save({k: v.clone() for k, v in model.state_dict().items()},
                       os.path.join(OUT, f"phase1_H{H}d_seed0.pt"))
        ...
        if n_anil > 0 and (has_crash or has_boom):
            ...
            model.train()
            for it in tqdm(range(n_anil), desc=f"ANIL H={H}d s{s_idx}", leave=False):
        ...
        if s_idx == 0:
            torch.save({k: v.clone() for k, v in model.state_dict().items()},
                       os.path.join(OUT, f"phase2_H{H}d_seed0.pt"))

        # Phase 3: Regime-gated TTA (crash only -- boom TTA disabled at
        # all horizons; see header for why).
        use_boom_tta = False
        ...
        seed_preds.append(dict(pred_raw=pred_raw, anil_pred_raw=anil_pred_raw,
                                pred_tta=pred_tta, pred_tta_bias=pred_tta_bias))
    ...
    def _seed_series(key, idx=None):
        vals = []
        for sp in seed_preds:
            p = sp[key]
            vals.append(_da(p, yte_raw, idx) if idx is not None else
                        float(np.mean(np.sign(p) == np.sign(yte_raw))))
        return vals

    seed_stats_H = {}
    for key, label in [("pred_raw","base"), ("anil_pred_raw","anil"),
                        ("pred_tta_bias","bias"), ("pred_tta","full")]:
        crash_vals = _seed_series(key, c_idx)
        boom_vals  = _seed_series(key, b_idx)
        all_vals   = _seed_series(key)
        seed_stats_H[label] = dict(
            crash_per_seed=crash_vals, crash_mean=float(np.mean(crash_vals)), crash_std=float(np.std(crash_vals)),
            boom_per_seed=boom_vals,   boom_mean=float(np.mean(boom_vals)),   boom_std=float(np.std(boom_vals)),
            overall_per_seed=all_vals, overall_mean=float(np.mean(all_vals)), overall_std=float(np.std(all_vals)),
        )
    SEED_STATS[H] = seed_stats_H

    print(f"  [Seed variance] crash DA full-TTA: {seed_stats_H['full']['crash_per_seed']} "
          f"-> mean={seed_stats_H['full']['crash_mean']:.3f} std={seed_stats_H['full']['crash_std']:.3f}")

    # Ensemble: average predictions across seeds
    pred_raw      = np.mean([sp["pred_raw"] for sp in seed_preds], axis=0)
    anil_pred_raw = np.mean([sp["anil_pred_raw"] for sp in seed_preds], axis=0)
    pred_tta      = np.mean([sp["pred_tta"] for sp in seed_preds], axis=0)
    pred_tta_bias = np.mean([sp["pred_tta_bias"] for sp in seed_preds], axis=0)
    pred_det = pred_raw - tte
    ...
    print(f"  [Ensemble, N={N_SEEDS}] Crash DA : {base_crash_da:.3f} -> {tta_crash_da:.3f}  {_pp(tta_crash_da, base_crash_da)}")
    print(f"  [Ensemble, N={N_SEEDS}] Boom  DA : {base_boom_da:.3f} -> {tta_boom_da:.3f}  {_pp(tta_boom_da, base_boom_da)}")
    print(f"  [Ensemble, N={N_SEEDS}] Overall  : DA {da_raw:.3f} -> {tta_all_da:.3f}  RMSE {rmse_raw:.5f} -> {tta_all_rmse:.5f}")
    print(f"  [Ensemble ablation] Base={base_crash_da:.3f}  ANIL-only={anil_crash_da:.3f}  "
    ...
    }).to_csv(os.path.join(OUT, f"predictions_H{H}d_ensemble.csv"), index=False)

with open(os.path.join(OUT, "seed_variance.json"), "w") as f:
    json.dump({str(k): v for k, v in SEED_STATS.items()}, f, indent=2)
print(f"\nSaved seed_variance.json (N_SEEDS={N_SEEDS} per horizon)")
# ═══════════════════════════════════════════════════════════════════
...
for H in HORIZONS:
    ...
    # Bias-only TTA significance (user request): H=5d's ablation showed
    # bias-only beating full-head — check whether that's real, not noise,
    # before treating it as a per-horizon design choice.
    res_crash_bias   = bootstrap_da_delta(d["pred"], d["pred_tta_bias"], d["actual"], c_idx, seed=H+100)
    res_overall_bias = bootstrap_da_delta(d["pred"], d["pred_tta_bias"], d["actual"], o_idx, seed=H+101)
    SIGNIFICANCE[H] = dict(crash=res_crash, overall=res_overall,
                            crash_bias=res_crash_bias, overall_bias=res_overall_bias)
    print(f"  H={H:>2}d  [full-head] crash DA delta={res_crash['delta']:+.3f}  "
          f"95% CI=[{res_crash['ci_lo']:+.3f}, {res_crash['ci_hi']:+.3f}]  "
          f"p(delta<=0)={res_crash['p_value']:.4f}  (n={res_crash['n']})")
    print(f"  H={H:>2}d  [bias-only] crash DA delta={res_crash_bias['delta']:+.3f}  "
          f"95% CI=[{res_crash_bias['ci_lo']:+.3f}, {res_crash_bias['ci_hi']:+.3f}]  "
          f"p(delta<=0)={res_crash_bias['p_value']:.4f}  (n={res_crash_bias['n']})")
