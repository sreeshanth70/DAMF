...
| `multiseed_adaptonce_H10.py` | Limitations, §8.3 | 3-seed re-run of the full pipeline (Phase 1 + ANIL + adapt-once TTA) at H=10d on the primary window, using the actual adapt-once policy (unlike the older daily-readaptation variant `patchtst_multiseed.py` used before it was fixed). Result in `results/seed_variance_adaptonce_H10.json`. Re-running the exact reported seed reproduced deterministically within one training session but diverged from the originally reported value across different sessions/hardware — consistent with GPU floating-point nondeterminism compounding over ANIL's small-batch updates; disclosed in the paper's Limitations. |
